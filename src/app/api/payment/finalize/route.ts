...
import { getMidtransConfig, isPaymentSuccessful, getMidtransTransactionStatus } from "@/lib/midtrans";
...
export async function POST(request: Request) {
  ...
  if (!midtrans.enabled || body.source === "mock") {
    success = true;
  } else {
    const status = await getMidtransTransactionStatus(body.orderId);
    success = isPaymentSuccessful(status);
  }
